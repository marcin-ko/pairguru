...
RSpec.describe Comment, type: :model do
  describe "validations" do
    ...
    context "when commenter already commented on the same movie" do
      let!(:other_comment) { create(:comment, commentable: movie, commenter: user) }
      ...
      context "and the other comment is not discarded" do
        it "is invalid" do
          expect(subject).not_to be_valid
      ...
      context "and the other comment is discarded" do
        before do
          other_comment.discard
        end
  ...
  describe "discarding comment" do
    ...
    it "doesn't remove the comment completely" do
      expect(comment.reload).to be_present
    end

    it "marks the comment as discarded" do
      expect(comment).to be_discarded
    end

    it "skips the comment when loading all kept comments" do
      expect(Comment.all).to include(comment)
      expect(Comment.kept).to be_empty
    end
  end
end
